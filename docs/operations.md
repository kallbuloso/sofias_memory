...
# LLM_API_KEY, EMBEDDING_API_KEY, COGNITIVE_IDEMPOTENCY_HMAC_KEY (a separate
# high-entropy secret, minimum 32 characters, never reuse API_KEY) in your
# shell environment or an env file passed to `docker compose --env-file`.
...
**v0.4.0 note:** this release introduces migration `0014` (Skills:
`skills`/`skill_revisions`, including the pgvector HNSW index used by
semantic resolve). Upgrading from a pre-v0.4.0 deployment therefore falls
under "Upgrading to a release with one or more new migrations" above — run
`alembic upgrade head` once, following §4, before starting the new image.
...
docker run --rm --network <compose-network> \
  ...
  sofias-memory:0.7.0 uv run --no-sync python scripts/rebuild_graph.py \
  --all --confirm-all
...
pulled per §A) for `sofias-memory:0.7.0`. Do not skip the migration step
...
- Prefer an exact version tag: `ghcr.io/kallbuloso/sofias-memory:0.7.0`.
