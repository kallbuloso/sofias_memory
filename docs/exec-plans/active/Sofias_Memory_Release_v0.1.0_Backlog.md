...
| REL-004 — CI mínima de qualidade e integração | DONE / validation pending publication | baseline `3c071c5`; workflows implementados e validados localmente, aguardando push + primeira execução real no GitHub (`ci.yml` automático + `integration.yml` manual) antes de DONE |
